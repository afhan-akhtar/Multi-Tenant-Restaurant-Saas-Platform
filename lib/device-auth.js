...
export function getDeviceRoutePath({ tenantId, deviceType, token }) {
  ...
  /** TABLET: token is globally unique — link without restaurant id in path */
  if (normalizedType === "TABLET") {
    return `/tablet?${new URLSearchParams({ token: t }).toString()}`;
  }
...
export async function validateDeviceAccess({ tenantId, token, deviceType }) {
  ...
  if (!normalizedTenantId || !normalizedToken || !["POS", "KDS", "TABLET"].includes(normalizedType)) {
    return null;
  }
  ...
  const tp = await getTenantPrisma(normalizedTenantId);
  const device = await tp.deviceToken.findFirst({
    where: {
  ...
  return { ...device, tenant: platformTenant };
}

/**
 * Resolve an active TABLET device by raw token only (tokenHash is unique).
 * Used for /tablet?token=… links without tenant id in the URL.
 */
export async function findTabletDeviceByToken(rawToken) {
  ...
  const tenants = await platformPrisma.tenant.findMany({
    where: { databaseUrl: { not: null }, status: "ACTIVE" },
    select: { id: true, name: true, subdomain: true, status: true },
  });

  const hash = hashDeviceToken(normalizedToken);
  for (const platformTenant of tenants) {
    const tp = await getTenantPrisma(platformTenant.id);
    const device = await tp.deviceToken.findFirst({
      where: {
        deviceType: "TABLET",
        status: "ACTIVE",
...
async function resolveBranchId(tenantId, branchId) {
  ...
  const fallbackBranch = await tp.branch.findFirst({
    where: { tenantId },
    orderBy: { id: "asc" },
    select: { id: true },
  });
...
export async function resolveDeviceStaffId(tenantId, branchId) {
  const tp = await getTenantPrisma(tenantId);
  const branchScopedStaff = await tp.tenantAdmin.findFirst({
    where: {
      tenantId,
      status: "ACTIVE",
      ...(branchId ? { branchId } : {}),
  ...
  const tenantStaff = await tp.tenantAdmin.findFirst({
    where: {
      tenantId,
      status: "ACTIVE",
    },
...
export async function getRequestActor(request, { allowedDeviceTypes = [] } = {}) {
  ...
  let staffId = null;
  if (requestedDeviceType === "POS") {
    staffId = await resolveDeviceStaffId(device.tenantId, branchId);
  }
  if (requestedDeviceType === "TABLET") {
    staffId = null;
  }
...
export function createDeviceSocketTicket({ tenantId, branchId = null, screenId = null, deviceType }) {
  ...
  if (!payload.tenantId || !["POS", "KDS", "TABLET"].includes(payload.deviceType)) {
