...
export async function syncKdsItemsForOrder({
  ...
  if (!tenantId || !orderId || !branchId || !Array.isArray(orderItems) || orderItems.length === 0) {
    return;
  }
...
export async function syncOrderKdsItemStatus(tenantId, orderId, orderStatus) {
...
export async function clearOrderKdsItems(tenantId, orderId) {
  if (!tenantId || !orderId) return;
  const prisma = await getTenantPrisma(tenantId);
