...
import { getRevenueByTenantFromOrders } from "./cross-tenant-aggregates";
import { formatEur } from "./currencyFormat";
...
export async function getDashboardData(tenantId = null, userType = "staff") {
  if (!tenantId) {
    return {
      ...
      metrics: {
        revenue: formatEur(0),
        taxCollected: formatEur(0),
        taxThisMonth: formatEur(0),
        todaySales: formatEur(0),
        todayTax: formatEur(0),
        salesReturn: formatEur(0),
        profit: formatEur(0),
        totalCustomers: 0,
        pendingOrders: 0,
        avgOrderValue: formatEur(0),
        totalProducts: 0,
        profitMargin: "0%",
      },
      paymentData: [],
      ...
      topCustomers: [],
      waiterPerformance: [],
      cashFlowData: [{ month: "N/A", sent: 0, received: 0 }],
    };
  }
  ...
  const [
    ...
    prisma.order.count({
      where: {
        ...tenantFilter,
        status: { in: ["OPEN", "CONFIRMED", "PREPARING", "READY", "PACK"] },
      },
    }),
  ...
  const salesReturn = toNum(refundedTotals._sum.amount);
  ...
  return {
    userType: "restaurant",
    metrics: {
      revenue: formatEur(revenue),
      taxCollected: formatEur(taxCollected),
      taxThisMonth: formatEur(taxThisMonth),
      todaySales: formatEur(todaySales),
      todayTax: formatEur(todayTax),
      salesReturn: formatEur(salesReturn),
      profit: formatEur(revenue - salesReturn),
      totalCustomers: customers,
      pendingOrders: pendingOrders,
      avgOrderValue: formatEur(avgOrderValue),
...
export async function getSuperAdminDashboardData() {
  const revenueByTenant = await getRevenueByTenantFromOrders();
  let platformOrderRevenue = 0;
  let platformOrderCount = 0;
  for (const v of Object.values(revenueByTenant)) {
    platformOrderRevenue += v.revenue;
    platformOrderCount += v.orderCount;
  }
  ...
  const [
    ...
    platformPrisma.tenantSubscription.count({
      where: { status: "ACTIVE" },
    }),
    ...
    platformPrisma.billingPayment.aggregate({
      _sum: { amount: true },
    }),
  ]);
  ...
  const pendingTenants = pendingTenantsCount;
  ...
  return {
    ...
    metrics: {
      ...
      platformRevenue: formatEur(toNum(platformOrderRevenue)),
      totalOrders: platformOrderCount,
      commissionEarned: formatEur(toNum(paymentStats._sum.amount || invoiceStats._sum.totalAmount)),
    },
    recentTenants: recentTenants.map((t) => ({
      name: t.name,
      subdomain: t.subdomain,
      status: t.status,
      orderCount: revenueByTenant[t.id]?.orderCount ?? 0,
    })),
    paymentData: [],
    salesByCategoryData: [{ name: "No data", sales: 0 }],
    topProducts: [],
