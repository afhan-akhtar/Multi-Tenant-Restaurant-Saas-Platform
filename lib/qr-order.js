...
import { verifyQrPaymentIntent } from "@/lib/payments/stripe";
import { getOrderSignature, signAndStoreOrder, signAndStorePayment } from "@/lib/tse/db";
...
/**
 * Authoritative subtotal / tax / total for QR checkout (must match PaymentIntent amount).
 */
export async function computeQrCheckoutTotals(tenantId, items) {
  ...
  return { lineInputs, subtotal, taxAmount, grandTotal };
}

/**
 * Create a dine-in order from the customer QR menu (no staff login). Uses same Order/KDS pipeline as POS/tablet.
 * @param {object} [stripePayment] - If set, card was charged in the browser; we verify and record Payment before KDS.
 */
export async function createQrTableOrder({
  ...
  const { lineInputs, subtotal, taxAmount, grandTotal } = await computeQrCheckoutTotals(tenantId, items);

  if (stripePayment?.paymentIntentId) {
    const existingPay = await prisma.payment.findFirst({
      where: {
        providerRef: String(stripePayment.paymentIntentId),
        method: "STRIPE",
        order: { tenantId, orderSource: "QR" },
      },
      include: { order: { include: { orderItems: true } } },
    });
    if (existingPay?.order) {
      const o = existingPay.order;
      const hasOrderTse = await getOrderSignature(tenantId, o.id);
      if (!hasOrderTse) {
        await signAndStoreOrder(tenantId, o.id, o.orderNumber, Number(o.grandTotal));
      }
      const kdsOrder = await getKDSOrderById(tenantId, o.id);
      return {
        order: o,
        orderNumber: o.orderNumber,
        qrClientToken: o.qrClientToken,
        kdsOrder,
        idempotentReplay: true,
      };
    }

    await verifyQrPaymentIntent({
      paymentIntentId: String(stripePayment.paymentIntentId),
      expectedAmount: grandTotal,
      tenantId,
      branchId,
      tableId: table.id,
      checkoutSessionId: String(stripePayment.checkoutSessionId || ""),
    });
  }
  ...
  if (stripePayment?.paymentIntentId) {
    const pi = String(stripePayment.paymentIntentId);
    await prisma.payment.create({
      data: {
        ...
        transactionId: pi,
        providerRef: pi,
        metadata: { channel: "browser", checkoutSessionId: String(stripePayment.checkoutSessionId || "") },
      },
    });
    await signAndStorePayment(tenantId, order.id, orderNumber, grandTotal, "STRIPE");
  }

  await signAndStoreOrder(tenantId, order.id, orderNumber, grandTotal);
