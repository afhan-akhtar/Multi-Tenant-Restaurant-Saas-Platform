...
import {
  captureMockPayPalOrder,
  createMockPayPalOrder,
  verifyMockPayPalCapture,
...
function buildCustomId({ tenantId, branchId, staffId }) {
  return `tenant:${tenantId}|branch:${branchId}|staff:${staffId}`;
}

export async function createPosPayPalOrder({ amount, tenantId, branchId, staffId }) {
  if (getPayPalMode() === "mock") {
    return createMockPayPalOrder({ amount, tenantId, branchId, staffId });
  }
      ...
      purchase_units: [
        {
          ...
          custom_id: buildCustomId({ tenantId, branchId, staffId }),
...
export async function capturePosPayPalOrder(orderId, context = {}) {
  if (String(orderId || "").startsWith("paypal_order_mock_")) {
    return captureMockPayPalOrder({ ...context, orderId });
  }
...
export async function verifyCapturedPayPalOrder({
  ...
  if (
    String(orderId || "").startsWith("paypal_order_mock_") ||
    String(captureId || "").startsWith("paypal_capture_mock_")
  ) {
    return verifyMockPayPalCapture({
      orderId,
      captureId,
      expectedAmount,
      tenantId,
  ...
  if (purchaseUnit.custom_id !== buildCustomId({ tenantId, branchId, staffId })) {
