...
function getSubscriptionExpectedMetadata({ tenantId, invoiceId, subscriptionId, checkoutSessionId }) {
  return {
...
function getQrExpectedMetadata({ tenantId, branchId, tableId, checkoutSessionId }) {
  return {
    integration: "restaurant_qr",
    tenantId: String(tenantId),
    branchId: String(branchId),
...
/** Guest table QR menu — card pay before order hits the kitchen. */
export async function createQrPaymentIntent({
  ...
  return stripe.paymentIntents.create({
    ...
    description: "Table QR dine-in order",
    metadata: getQrExpectedMetadata({ tenantId, branchId, tableId, checkoutSessionId }),
  });
}

export async function verifyQrPaymentIntent({
  paymentIntentId,
  expectedAmount,
  tenantId,
  branchId,
  ...
  const expectedMetadata = getQrExpectedMetadata({
    tenantId,
    branchId,
    tableId,
    checkoutSessionId,
  ...
  if (paymentIntent.currency !== currency) {
    throw new Error("Stripe payment currency does not match.");
  }
  ...
  if (!paymentIntent.payment_method_types?.includes("card")) {
    throw new Error("Table QR payment must use a card PaymentIntent.");
  }

  for (const [key, value] of Object.entries(expectedMetadata)) {
    if (metadata[key] !== value) {
      throw new Error("Stripe payment does not belong to this table order.");
    }
  }
...
export async function createSubscriptionPaymentIntent({
  ...
  return stripe.paymentIntents.create({
    ...
    payment_method_types: ["card"],
    capture_method: "automatic",
    description: "Restaurant SaaS subscription payment",
    metadata: getSubscriptionExpectedMetadata({
      tenantId,
      invoiceId,
      subscriptionId,
      checkoutSessionId,
...
export async function verifySubscriptionPaymentIntent({
  paymentIntentId,
  expectedAmount,
  tenantId,
  invoiceId,
  ...
  const currency = getPaymentCurrency().toLowerCase();
  const metadata = paymentIntent.metadata || {};
  const expectedMetadata = getSubscriptionExpectedMetadata({
    tenantId,
    invoiceId,
    subscriptionId,
    checkoutSessionId,
  ...
  if (paymentIntent.currency !== currency) {
    throw new Error("Stripe payment currency does not match billing currency.");
  }

  if (paymentIntent.amount_received !== toMinorUnits(expectedAmount)) {
    throw new Error("Stripe payment amount does not match invoice total.");
  }

  if (!paymentIntent.payment_method_types?.includes("card")) {
    throw new Error("Subscription payment must use a browser card PaymentIntent.");
  }

  for (const [key, value] of Object.entries(expectedMetadata)) {
    if (metadata[key] !== value) {
      throw new Error("Stripe payment does not belong to this subscription invoice.");
    }
  }
...
export async function refundPosPaymentIntent(input) {
  const stripe = getStripeClient();
  const {
    paymentIntentId,
    amount = null,
    metadata = {},
    idempotencyKey = null,
  } = typeof input === "string" ? { paymentIntentId: input } : (input || {});
  const paymentIntent = await stripe.paymentIntents.retrieve(paymentIntentId);
  ...
  const normalizedMetadata = Object.fromEntries(
    Object.entries({
      integration: "restaurant_pos_refund",
      ...metadata,
  ...
  const refundPayload = {
    ...
    ...(amount != null ? { amount: toMinorUnits(roundMoney(amount)) } : {}),
  };
