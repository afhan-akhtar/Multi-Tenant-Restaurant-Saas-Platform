...
export async function fiskalySignTransaction(payload) {
  ...
  try {
    ...
    const logEntry = tx.log ?? tx;
    const sig = logEntry?.signature ?? tx?.log?.signature ?? tx;
    const sigVal = (typeof sig === "object" && sig !== null && (sig.value ?? sig.signature))
      ? (sig.value ?? sig.signature)
      : (typeof tx?.signature === "string" ? tx.signature : tx?.serial_number ?? "");
    const result = {
      transactionId: tx.number ?? tx.transaction_number ?? tx.id ?? txId,
      signature: typeof sigVal === "string" ? sigVal : "",
      timestamp: tx.time_end ?? logEntry?.time_end ?? tx.timestamp ?? new Date().toISOString(),
      tssSerialNumber: tx.tss_serial_number ?? logEntry?.tss_serial_number,
      qrCodeData: tx.qr_code_data ?? logEntry?.qr_code_data,
      provider: "fiskaly",
    };
