...
export async function resolveCred(tenantId) {
  ...
  if (tenantId != null) {
    try {
      const tp = await getTenantPrisma(Number(tenantId));
      const row = await tp.tenantFiskalyConfig.findUnique({
  ...
  if (apiKey && apiSecret) {
    try {
      const row = await platformPrisma.fiskalyPlatformConfig.findUnique({
        where: { apiKey },
      });
...
export async function fiskalySignTransaction(payload) {
  ...
  // DSFinV-K / Kassenbeleg-V1 compatible processData (sale and TSS refund / storno):
  // Beleg^<Umsatz_19>_<Umsatz_7>_<Umsatz_10_7>_<Umsatz_5_5>_<Umsatz_0>^<Zahlbetrag>_<type>_<currency>[...]
  // Refunds use the same process_type + first field as a normal receipt, with negative amounts in
  // buckets and payments. Fiskalcheck rejects Storno-V1 / AVBelegstorno pairs in the QR (processType
  // and processData type checks); TSS-connected cancellations are represented as negative Kassenbeleg-V1.
  ...
  /**
   * DSFinV-K Kassenbeleg payment order: Bar EUR, Bar FX (alpha), Unbar EUR, Unbar FX (alpha).
   * Negative amounts are allowed for refund lines.
   */
  const sortPaymentsForProcessData = (list) => {
    const rank = (p) => {
      const rawType = (p.type || "BAR").toUpperCase();
      const isBar = rawType === "CASH" || rawType === "BAR";
      const cur = (p.currency || "EUR").toUpperCase();
      if (isBar && cur === "EUR") return { g: 0, cur: "" };
      if (isBar) return { g: 1, cur };
      if (cur === "EUR") return { g: 2, cur: "" };
      return { g: 3, cur };
    };
    return [...list].sort((a, b) => {
      ...
      return String(ra.cur).localeCompare(String(rb.cur));
    });
  };
  ...
  const paymentSection = sortPaymentsForProcessData(payments)
    .map((p) => {
  ...
  const processPayload = [payload.fn || "Beleg", taxBucket, paymentSection].join("^");
