...
export async function submitCashPointClosing(opts) {
  ...
  try {
    ...
    for (const t of tseTx) {
      ...
      const refundAbs = round2(Math.abs(Number(t.rawPayload?.refundAmount ?? 0)));
      const saleAmount = round2(t.rawPayload?.amount ?? t.rawPayload?.grandTotal ?? 0);
      const amount = isStorno ? -refundAbs : saleAmount;
      const txExportId = str40(String(t.id));
      const ts = new Date(t.signedAt ?? Date.now());
      const tsSec = Math.floor(ts.getTime() / 1000);
      const head = {
        type: "Beleg",
        name: isStorno ? "AVBelegstorno" : "Beleg",
        storno: isStorno,
        number: int(t.id),
          ...
          lines: [
            {
              ...
              storno: isStorno,
              text: isStorno ? "AVBelegstorno" : "Beleg",
              business_case: {
                type: "Umsatz",
                name: isStorno ? "AVBelegstorno" : "Beleg",
...
export async function triggerDsfinvkExport(opts) {
  const clientId = opts.clientId || "pos-1";
  const format = opts.format === "tar" ? "tar" : "zip";
  const businessDateStart = opts.businessDateStart || opts.businessDate || new Date().toISOString().slice(0, 10);
  const businessDateEnd = opts.businessDateEnd || opts.businessDate || businessDateStart;

  const token = await getDsfinvkToken();
  await ensureCashRegister(token, clientId);

  const exportId = randomUUID();
  const url = `${DSFINVK_URL}/exports/${exportId}`;
  const payload = {
    format,
    client_id: clientId,
    business_date_start: businessDateStart,
    business_date_end: businessDateEnd,
  };
  ...
  const text = await res.text();
  if (!res.ok) throw new Error(`DSFinV-K export trigger: ${res.status} ${text}`);
  ...
  const pollUntilDone = async () => {
    ...
    while (Date.now() < deadline) {
      ...
      if (state === "ERROR") {
        const err = body.error ?? body.message ?? body.error_message ?? body;
        throw new Error(`DSFinV-K export failed: ${typeof err === "string" ? err : JSON.stringify(err)}`);
      }
    }
    throw new Error("DSFinV-K export timed out");
  };

  const final = await pollUntilDone();
  return { exportId, format, state: final.state };
}

export async function downloadDsfinvkExport(opts) {
  const exportId = opts.exportId;
  if (!exportId) throw new Error("Missing exportId");
  const token = await getDsfinvkToken();
  const url = `${DSFINVK_URL}/exports/${exportId}/download`;
  const res = await fetch(url, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${token}`,
      Accept: opts.format === "tar" ? "application/x-tar" : "application/zip",
    },
  });
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`DSFinV-K export download: ${res.status} ${text}`);
  }
  const buf = Buffer.from(await res.arrayBuffer());
  const contentType = res.headers.get("content-type") || (opts.format === "tar" ? "application/x-tar" : "application/zip");
