...
function verifyImpersonateToken(token) {
  try {
    ...
    return {
      ...
      exp: payload.exp,
      saId: payload.saId ?? null,
      saEmail: payload.saEmail ?? null,
    };
  } catch {
    return null;
  }
  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        const superAdminIssued = Boolean(payload.saId || payload.saEmail);
        return {
          id: String(staff.id),
          email: staff.email,
          name: staff.name,
          ...
          impersonation: superAdminIssued,
          impersonationSuperAdminId: superAdminIssued ? payload.saId : null,
          impersonationSuperAdminEmail: superAdminIssued ? payload.saEmail : null,
          impersonationExpiresAt: payload.exp ?? Date.now() + 60 * 60 * 1000,
        };
      },
    }),
    CredentialsProvider({
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        if (user.impersonation) {
          ...
          token.impersonationSuperAdminId = user.impersonationSuperAdminId ?? null;
          token.impersonationSuperAdminEmail = user.impersonationSuperAdminEmail ?? null;
          const expMs = user.impersonationExpiresAt ?? Date.now() + 60 * 60 * 1000;
          token.impersonationExpiresAt = expMs;
          token.exp = Math.floor(expMs / 1000);
        } else {
          token.impersonation = false;
          delete token.impersonationSuperAdminId;
          delete token.impersonationSuperAdminEmail;
    ...
    async session({ session, token }) {
      if (session?.user) {
        ...
        session.user.impersonation = Boolean(token.impersonation);
        session.user.impersonationSuperAdminId = token.impersonationSuperAdminId ?? null;
        session.user.impersonationSuperAdminEmail = token.impersonationSuperAdminEmail ?? null;
