...
export async function calculateSubscriptionCharges(
  ...
  const revenueAggregate = await tenantPrisma.order.aggregate({
    where: {
      tenantId,
      status: "COMPLETED",
      createdAt: {
...
export async function createInvoiceForSubscription(
  subscription,
  options = {},
  platformDb = platformPrisma
) {
  const plan =
    subscription.plan ||
    (await platformDb.subscriptionPlan.findUnique({
  ...
  const existing = await getCurrentPeriodInvoice(platformDb, subscription.id, periodStart, periodEnd);
  if (existing) {
    return existing;
  }

  const tenantPrisma = await getTenantPrisma(subscription.tenantId);
  ...
  return platformDb.billingInvoice.create({
    data: {
...
export async function syncSubscriptionState(dbClient, subscriptionId, now = new Date()) {
  ...
  if (!latestInvoice) {
    latestInvoice = await createInvoiceForSubscription(subscription, {}, dbClient);
  }
...
export async function runSubscriptionBillingCycle(dbClient = platformPrisma) {
  ...
  return {
    ...
    plan: subscription.plan
      ? {
          ...
          features: {
            ...buildPlanFeatures(featureCodes, featureLabels),
            ...(rawPlanFeat.commissionModel ? { commissionModel: rawPlanFeat.commissionModel } : {}),
            ...(rawPlanFeat.flatFeePerOrder != null ? { flatFeePerOrder: rawPlanFeat.flatFeePerOrder } : {}),
          },
          featureMatrix,
        }
      : null,
...
export async function getTenantSubscriptionAccess(tenantId) {
  const subscription = await syncTenantSubscription(platformPrisma, tenantId);
