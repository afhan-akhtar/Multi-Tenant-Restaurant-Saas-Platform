...
export async function getSalesReport(tenantId, startDate, endDate) {
  // Use UTC boundaries; avoids JS "YYYY-MM-DD" timezone pitfalls.
  const start = startOfDayUtc(startDate);
  const end = endOfDayUtc(endDate);

  // Include CONFIRMED orders too: POS creates CONFIRMED first; KDS later moves to COMPLETED.
  const where = { tenantId, status: { in: ["CONFIRMED", "COMPLETED"] }, createdAt: { gte: start, lte: end } };
...
export async function getZReportData(tenantId, date) {
  ...
  const serializedOrders = orders.map((o) => ({
    ...
    payments: (o.payments || []).map((p) => ({ ...p, amount: toNum(p.amount) })),
  }));

  const serializedCashbook = cashbook.map((c) => ({ ...c, amount: toNum(c.amount) }));
  ...
  return {
    ...
    paymentBreakdown,
    cashbookEntries: serializedCashbook,
  };
}
