...
export const auth = {
  ...
  title: "m-0 mb-1 text-2xl font-bold tracking-tight text-slate-900",
  subtitle: "m-0 mb-6 text-sm text-slate-500",
  label: "mb-1 block text-sm font-semibold text-slate-700",
  input:
    "w-full rounded-xl border border-stone-300 bg-white px-4 py-3 text-base text-slate-900 shadow-sm outline-none ring-teal-500/15 placeholder:text-slate-400 box-border focus:border-teal-500 focus:ring-2 font-medium",
  select:
  ...
  btnPrimary:
    "inline-flex w-full items-center justify-center rounded-full border-0 bg-gradient-to-r from-teal-600 to-teal-700 py-3 text-base font-bold text-white shadow-md shadow-teal-600/25 transition hover:from-teal-500 hover:to-teal-600 hover:shadow-lg hover:shadow-teal-500/30 disabled:cursor-not-allowed disabled:opacity-70",
  link: "font-semibold text-teal-700 no-underline hover:text-teal-800 hover:underline",
  muted: "text-sm text-slate-500",
  error: "text-sm text-red-600",
};

export const authDisplayFont = "font-bold";

export function AuthShell({ children }) {
  return (
    <div
      className="relative min-h-screen min-h-[100dvh] box-border overflow-x-hidden bg-[#f6f4f0] font-poppins text-slate-800 antialiased"
    >
