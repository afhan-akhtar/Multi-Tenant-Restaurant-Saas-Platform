...
export default function ConfirmModal({ open, title, message, confirmLabel = "Delete", onConfirm, onCancel, loading = false }) {
  if (!open) return null;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50">
      <div className="w-full max-w-sm bg-color-card rounded-xl shadow-xl border border-color-border p-6">
        ...
        <p className="m-0 mb-6 text-sm text-color-text-muted">{message}</p>
